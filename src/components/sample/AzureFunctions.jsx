...
async function callFunction(teamsUserCredential, functionName) {
...
export function AzureFunctions(props) {
  ...
  const { codePath, docsUrl } = {
    codePath: `api/getUserProfile/index.js`,
  ...
  const { loading, data, error, reload } = useData(async () => {
    ...
    try {
      const functionRes = await callFunction(teamsUserCredential, "getUserProfile");
      return functionRes;
    } catch (error) {
